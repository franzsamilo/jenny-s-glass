        ...
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.12}>
            <p className="mt-5 text-[16px] leading-relaxed text-ink-soft">
              ...
              materials — so one store handles the whole job.
            </p>
          </Reveal>
        </div>
          ...
          {pillars.map((p, i) => (
            <Reveal key={p.id} delay={0.06 * i}>
              <article className="framed framed-hover h-full overflow-hidden flex flex-col">
                <div className={`swatch ${p.swatch} h-28 relative`}>
                  <span className="absolute right-3 top-3 mono-data text-[10px] tracking-[0.18em] text-white/80">{p.no}</span>
                </div>
                <div className="p-6 flex flex-col flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="display display-md" style={{ fontSize: 22 }}>{p.title}</h3>
                  </div>
